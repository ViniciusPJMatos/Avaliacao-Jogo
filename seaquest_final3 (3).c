...
static int invulneravel = 0;
static int invulneravel_timer = 0;
...
#define VELOCIDADE_TIRO_INIMIGO   1
...
static void atualizar_tiros_inimigos(void) {
    for (int i = 0; i < MAX_TIROS_INIMIGOS; i++) {
        if (tiros_inimigos[i].ativo) {
            ...
            else if (!invulneravel && sobrepoe(jogador.x, jogador.y, JOGADOR_LARG, JOGADOR_ALT,
...
static void atualizar_inimigos(void) {
    for (int i = 0; i < MAX_INIMIGOS; i++) {
        ...
        if (!invulneravel && sobrepoe(jogador.x, jogador.y, JOGADOR_LARG, JOGADOR_ALT,
...
static void desenhar(void) {
    ...
    for (int l = 0; l < TOTAL_LINHAS; l++) {
        ...
        for (int c = 0; c < LARGURA && pos < (int)(sizeof(linha) - 1); c++) {
            int cor = cores[l][c];
            if (cor != cor_atual) {
                const char *codigo = codigo_cor(cor);
                int escrito = snprintf(linha + pos, sizeof(linha) - pos, "%s", codigo);
                if (escrito < 0 || escrito >= (int)(sizeof(linha) - pos)) break;
                pos += escrito;
                cor_atual = cor;
            }
            linha[pos++] = tela[l][c];
        ...
        if (pos < (int)(sizeof(linha) - 1)) {
            int escrito = snprintf(linha + pos, sizeof(linha) - pos, "\033[0m\033[K\n");
            if (escrito > 0 && escrito < (int)(sizeof(linha) - pos)) pos += escrito;
        }

        fwrite(linha, 1, pos, stdout);
    }
    fflush(stdout);
}
...
int main(void) {
    ...
    while (1) {
        ...
        while (!game_over) {
            ...
            if (invulneravel) {
                invulneravel_timer--;
                if (invulneravel_timer <= 0) invulneravel = 0;
            }
            desenhar();
